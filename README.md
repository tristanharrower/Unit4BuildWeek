...
|GET     | /potlucks   | Header -> Authorization : token, Body -> person_id    | potluck_id, person_id, event_name, description, event_date, event_time, location| gets all organized potlucks, if person_id in req.body, returns users organized potlucks |
...
|Method   | Endpoint      |Request    | Response  | Notes  |
|------   | -----------   | ------- | ------- | ------ |
|POST     | /attendingPotlucks   | Header -> Authorization : token, Body -> person_id, username, event_name, description, event_date, event_time, location, role     |potluck_id, person_id, username, event_name, description, event_date, event_time, location, role| attends a potluck for a specific user|
|GET     | /attendingPotlucks   | Header -> Authorization : token, Body -> person_id    | potluck_id, person_id, event_name, description, event_date, event_time, location| gets all attending potlucks, if person_id in req.body, returns users attending potlucks |
|DELETE     | /attending-potlucks/:potluckid   |  Header -> Authorization:token, Body -> person_id     | User req.body.person_id no longer attending Potluck: req.params.potluckid| deletes single attending potluck  |
---
