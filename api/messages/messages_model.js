...
async function createMessage(message) {
    const [newMessage] = await db('messages').insert(message,
        ['message_id',  'organizer_id', 'attendee_id', 'attendee_username', 'potluck_id'] )
    return newMessage 
  }
  ...
  async function findBy(filter) {
    ...
    if(filterKey[0]===undefined){
      ...
      .select('messages.message_id','potlucks.potluck_id','potlucks.username',  'messages.attendee_id',
         'messages.attendee_username','potlucks.event_name', 'potlucks.description','potlucks.event_date', 
